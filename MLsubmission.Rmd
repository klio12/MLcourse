...
* Brute force approach with the highly-accurate random forest algorithm using all variables ran too long  
...
**Step 2**: No variables were found to have near-zero variance
```{r s2, cache=TRUE}
nZV=nearZeroVar(tv, saveMetrics=T)
rownames(nZV[nZV$nzv==TRUE, ])
```

Training set was split data into a **training set** and a **validation set**, in ratio 3:1
```{r spl, cache=TRUE}
...
Another prediction approach that was attempted was classification tree with **bagging**, which achieved high accuracy as expected, but at the expense of a longer run (*370 sec*)
```{r bag, cache=TRUE}
set.seed(456)
